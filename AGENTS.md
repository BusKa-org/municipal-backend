...
Run `make help` for the full target list — the Makefile has grown past what's
listed above (migrations, docker targets, etc.).

Integration and e2e tests need Postgres/PostGIS reachable (`make db-up`, or
`make run` if already configured). If `pytest` fails in pre-commit with a
connection error, that is almost always a missing local database, not a code
problem.
...
| Why was a non-obvious decision made? | `docs/adr/` — check the index before assuming something is undocumented. **Not on `main` yet as of this writing**, landing with the load-testing PR; if it's missing, no ADRs have been merged yet |
| What does an endpoint accept/return? | [`docs/api_contracts.md`](docs/api_contracts.md) (currently a stub), [`docs/endpoints/`](docs/endpoints/), or the running Swagger UI at `/docs` |
| How does the load-testing harness work? | `docs/adr/0001-tiered-load-testing-harness.md`, `loadtest/README.md` — same caveat, not merged to `main` yet |
