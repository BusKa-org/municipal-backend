...
class RotasService:
    ...
    @staticmethod
    def listar_rotas_para_aluno(aluno_id: str) -> list[dict]:
        """
        List routes for student with subscription status.
    ...
    @staticmethod
    def inscrever_aluno(aluno_id: str, rota_id: str) -> dict:
        """
        Subscribe a student to a route.
        
        Raises: NotFoundError, ForbiddenError, AppError
        """
        aluno = Aluno.query.get(aluno_id)
        rota = Rota.query.get(rota_id)
        
        if not aluno or not rota:
            raise NotFoundError("Dados inválidos")
        
        if aluno.prefeitura_id != rota.prefeitura_id:
            raise ForbiddenError("Rota pertence a outra prefeitura")
        ...
        try:
            ...
            db.session.rollback()
            raise AppError(f"Erro ao inscrever aluno: {str(e)}", 500)

    @staticmethod
    def sair_da_rota(aluno_id: str, rota_id: str) -> None:
        """
        Unsubscribe a student from a route.
        ...
        try:
            ...
            db.session.rollback()
            logger.error(f"Error unsubscribing student: {e}")
            raise AppError(f"Erro ao remover inscrição: {str(e)}", 500)
